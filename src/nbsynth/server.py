...
def serve():
    nbsynth_socket = socket.socket(family=socket.AF_INET, type=socket.SOCK_STREAM, proto=0, fileno=None)
    nbsynth_socket.bind((hostname, port_no))
    nbsynth_socket.listen(5)
    nbsynth = defaultdict(NBSynth)
    shutdown_condition = False

    while True:
        client, client_addr = nbsynth_socket.accept()
        ...
        if event:
            result = nbsynth[notebook_name].execute_event(event)
        else:
            client.send("Server is stopping...\n".encode())
            nbsynth_socket.close()
            break
        ...
        if event_str == "close_notebook":
            nbsynth.pop(notebook_name)
            if shutdown_condition and not len(nbsynth.keys()):
