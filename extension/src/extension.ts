...
function activate(context: vscode.ExtensionContext) {
	let nbsynthDiagnostics = vscode.languages.createDiagnosticCollection("NBSynth");
	context.subscriptions.push(nbsynthDiagnostics);
	let notebookEvents = new Events(new ServerHandler(nbsynthDiagnostics));
	...
	context.subscriptions.push(vscode.commands.registerCommand("nbsynthvscode.filterResults", async() => notebookEvents.chooseAnalyses()));
