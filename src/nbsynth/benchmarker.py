...
def benchmark(folder, analyses, level, output, filter, notebooks_to_run = None):
    ...
    for f in tqdm(dir_list):
        ...
        nbsynth = NBSynth(level=level, filename=f)
        try:
        ...
        for start in nbsynth.notebook_IR.keys():
            nbsynth.add_analyses(analyses)

            event = RunBatchEvent(start)
            results = nbsynth.execute_event(event).dumps(True)

        for akey in nbsynth.active_analyses:
            for s in nbsynth.all_analyses[akey].stats:
...
def main():
    parser = ArgumentParser(description="NBSynth benchmarker version 1.0 ")
