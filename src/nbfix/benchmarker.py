...
def benchmark(folder, analyses, level, output, filter, notebooks_to_run = None):
    ...
    for f in tqdm(dir_list):
        ...
        nbfix = NBFix(level=level, filename=f)
        try:
        ...
        for start in nbfix.notebook_IR.keys():
            nbfix.add_analyses(analyses)

            event = RunBatchEvent(start)
            results = nbfix.execute_event(event).dumps(True)

        for akey in nbfix.active_analyses:
            for s in nbfix.all_analyses[akey].stats:
...
def main():
    parser = ArgumentParser(description="NBFix benchmarker version 1.0 ")
