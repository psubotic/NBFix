...
class DetectBugsEvent(Event):
    ...
    def execute(self, nbsynth):
        if self.scope == "full":
            context = build_full_notebook_context(nbsynth.notebook_IR)
            ...
            context = _CONTEXT_BUILDERS[self.scope](nbsynth.notebook_IR, self.cell_index)
        else:
            raise ValueError(f"Unknown scope: {self.scope!r}")

        findings_json = self.client.chat_json(SYSTEM_PROMPT, build_user_prompt(context))
        return map_findings_to_result(findings_json, nbsynth.notebook_IR)
