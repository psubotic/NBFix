...
import { INBSynthDiagnostic } from './handler';
...
/**
 * Converts one cell's NBSynth error list into CodeMirror Diagnostic ranges,
...
export function toCodeMirrorDiagnostics(
  view: EditorView,
  cellDiagnostic: INBSynthDiagnostic
): Diagnostic[] {
...
/**
 * Registers the NBSynth linter as a global editor extension so any code
 * editor JupyterLab creates (including every notebook cell) can display
 * diagnostics pushed via diagnosticsStore.set().
 */
export function registerNBSynthLinter(registry: IEditorExtensionRegistry): void {
  registry.addExtension({
    name: 'nbsynth:diagnostics',
    factory: () => {
      const source = (view: EditorView): Diagnostic[] => diagnosticsStore.get(view);

      const nbsynthLinter = linter(source, {
        delay: 0,
        needsRefresh: update => {
