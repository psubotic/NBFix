...
def load_notebook_resilient(cells_json) -> dict[int, IntermediateRepresentations]:
    ...
    - the core engine (analyzer.py's NBSynth.load_notebook,
    ...
    nbsynth.notebook_IR) is untouched and still fails fast on the same
    input. A best-effort partial view is useful for ad-hoc LLM checks and
    benchmarking over real-world notebooks that may use constructs NBSynth
