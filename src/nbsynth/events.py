...
def reindex_results(nbsynth: NBSynth, updated_index: int, added = True):
    for result in nbsynth.results.values():
...
class AddActiveAnalysesEvent(Event):
    ...
    def execute(self, nbsynth):
        nbsynth.add_analyses(self.active_analyses)
        return nbsynth.run_analyses(-1, [IDLE, ISOLATED]).join_by_cell_id()
...
class OpenNotebookEvent(Event):
    ...
    def execute(self, nbsynth):
        nbsynth.load_notebook(self.notebook_json)
...
class RunCellEvent(Event):
    ...
    def execute(self, nbsynth):
        results = nbsynth.run_analyses(self.cell_index, [STALE, DATA_LEAK])
        nbsynth.update_abstract_states(self.cell_index)
        nbsynth.notebook_IR[self.cell_index].last_ran_code = nbsynth.notebook_IR[self.cell_index].cell_code
...
class RunBatchEvent(Event):
    ...
    def execute(self, nbsynth):
        return nbsynth.run_analyses(self.start_cell, detailed = True)
...
class AddCellEvent(Event):
    ...
    def execute(self, nbsynth: NBSynth):
        new_notebook_IR: dict[int, IntermediateRepresentations] = defaultdict()
        all_keys = list(nbsynth.notebook_IR.keys())
        max_key = max(all_keys)
        for i in range(len(all_keys)):
            if all_keys[i] >= self.position:
                new_notebook_IR[all_keys[i] + 1] = nbsynth.notebook_IR[all_keys[i]]
                new_notebook_IR[all_keys[i] + 1].cell_id = all_keys[i] + 1
            if all_keys[i] < self.position:
                new_notebook_IR[all_keys[i]] = nbsynth.notebook_IR[all_keys[i]]
        
        if self.position <= max_key:
            new_notebook_IR[max_key + 1] = nbsynth.notebook_IR[max_key]
        ...
        nbsynth.notebook_IR = new_notebook_IR
        reindex_results(nbsynth, self.position)
        return nbsynth.run_analyses(-1, [IDLE, ISOLATED]).join_by_cell_id()
...
class RemoveCellEvent(Event):
    ...
    def execute(self, nbsynth: NBSynth):
        new_notebook_IR: dict[int, IntermediateRepresentations] = defaultdict()
        for index in nbsynth.notebook_IR.keys():
            if index > self.position:
                new_notebook_IR[index - 1] = nbsynth.notebook_IR[index]
                new_notebook_IR[index - 1].cell_id = index - 1
            if index < self.position:
                new_notebook_IR[index] = nbsynth.notebook_IR[index]
        
        nbsynth.notebook_IR = deepcopy(new_notebook_IR)
        reindex_results(nbsynth, self.position, False)
        return nbsynth.run_analyses(-1, [IDLE, ISOLATED]).join_by_cell_id()
...
class ChangeCellCodeEvent(Event):
    ...
    def execute(self, nbsynth: NBSynth):
        if self.cell_index in nbsynth.notebook_IR.keys():
            last_ran_code = nbsynth.notebook_IR[self.cell_index].last_ran_code
            nbsynth.notebook_IR[self.cell_index] = IntermediateRepresentations(self.new_code, self.cell_index, last_ran_code)
        if self.with_result:
            return nbsynth.run_analyses(-1, [IDLE, ISOLATED]).join_by_cell_id()
