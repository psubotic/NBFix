    ...
    def execute(self, nbfix):
        ...
        if self.scope == "full":
            context = build_full_notebook_context(
                nbfix.notebook_IR, deterministic_findings, dependency_edges=self.dependency_edges,
            )
            ...
            context = _CONTEXT_BUILDERS[self.scope](
                nbfix.notebook_IR, self.cell_index, deterministic_findings, dependency_edges=self.dependency_edges,
            )
        else:
