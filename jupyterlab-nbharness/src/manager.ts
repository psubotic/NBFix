...
import { INBHarnessDiagnostic, postEvent } from './handler';
...
// tracked in NBHarness's notebook_IR, so newly added cells are always
...
// NBHarness.all_analyses entries the backend's active_analyses list
...
function mergeDiagnostics(
  a: INBHarnessDiagnostic[],
  b: INBHarnessDiagnostic[]
): INBHarnessDiagnostic[] {
  const byCellId = new Map<number, INBHarnessDiagnostic['errors']>();
...
function countFindings(diagnostics?: INBHarnessDiagnostic[]): number {
  return (diagnostics ?? []).reduce((sum, cellDiagnostic) => sum + cellDiagnostic.errors.length, 0);
}

/**
 * Wires one NotebookPanel's lifecycle/edit/execute signals to the NBHarness
...
export class NotebookSessionManager {
  ...
  private _deterministicDiagnostics: INBHarnessDiagnostic[] = [];
  private _llmDiagnostics: INBHarnessDiagnostic[] = [];
  private _llmStaleDiagnostics: INBHarnessDiagnostic[] = [];
  private _llmApiSequenceDiagnostics: INBHarnessDiagnostic[] = [];
  ...
  private _warnLLMStaleFailure(error: unknown): void {
    console.error('nbharness: LLM stale-cell check failed', error);
    if (this._llmStaleWarned) {
      return;
    }
    this._llmStaleWarned = true;
    Notification.warning(
      `NBHarness: LLM stale-cell detection is enabled but failed (${String(error)}). ` +
  ...
  private _warnLLMApiSequenceFailure(error: unknown): void {
    console.error('nbharness: LLM API-sequence check failed', error);
    if (this._llmApiSequenceWarned) {
      return;
    }
    this._llmApiSequenceWarned = true;
    Notification.warning(
      `NBHarness: LLM API call-sequence detection is enabled but failed (${String(error)}). ` +
        'Check that the llm extra is installed and its configured endpoint is reachable. ' +
        'Not shown again this session.',
      { autoClose: 8000 }
    );
  ...
  private _setDeterministicDiagnostics(diagnostics?: INBHarnessDiagnostic[]): void {
    if (diagnostics) {
  ...
  private _setLLMDiagnostics(diagnostics?: INBHarnessDiagnostic[]): void {
    if (diagnostics) {
  ...
  private _setLLMStaleDiagnostics(diagnostics?: INBHarnessDiagnostic[]): void {
    if (diagnostics) {
